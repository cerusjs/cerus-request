...
module.exports = class request {
	...
	cookie(key, value) {
		...
		let cookies = this._options["headers"]["cookie"];
	...
	send(end_function) {
		...
		if(this._options["body"]) {
			...
			this._options["headers"]["content-length"] = this._options["headers"]["content-length"] || this._options["body"].length;
		}
		
		const req = http.request(this._options, response => {
			let body = "";
			...
			response.on("end", () => {
				this._run_queue(response, body, end_function);
			});
		});

		req.on("error", () => {
		 	this._ended(err, end_function);
		});
		
		if(this._options["body"]) {
			req.write(this._options["body"]);
		}
